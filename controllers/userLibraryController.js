...
module.exports = {
  ...
  getUserLibraryByAnalytic: (spotifyID, filter) => {
    ...
    return new Promise((resolve, reject) => {
      connection.query(
        `SELECT * FROM library JOIN UserLibrary ON library.song_id = UserLibrary.song_id WHERE library.song_id IN (SELECT song_id FROM UserLibrary WHERE user_id = ?) ORDER BY ${filter} DESC LIMIT 20`,
        [spotifyID],
        (err, data) => {
          if (err) reject(new ServerError('UserLibrary - getUserLibrary failed'));
          // data.forEach((item) => {
